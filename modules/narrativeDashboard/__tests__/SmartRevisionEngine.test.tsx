...
jest.mock('../components/RevisionHistoryPanel', () => ({
  __esModule: true,
  default: ({ revisions }: any) => <div data-testid="revision-history">{revisions.length} revisions</div>,
}));
...
jest.mock('../../shared/state/useNarrativeSyncContext', () => ({
  useNarrativeSync: () => ({ state: {}, setScene: jest.fn(), setCharacter: jest.fn() })
}));
...
const sampleSuggestion: OptimizationSuggestion = {
  ...
  expectedImpact: { tensionChange: 10, empathyChange: 0, engagementChange: 15, complexityChange: 0 },
  targetPositions: [0.5],
  riskLevel: 'medium',
...
const diffSegments = [
  ...
  { text: ' ', type: 'unchanged' },
  { text: 'through the hall, heart pounding.', type: 'added', semanticTag: 'pacing' },
...
describe('<SmartRevisionEngine />', () => {
  it('renders side-by-side preview', async () => {
    render(<SmartRevisionEngine sceneId="scene1" suggestion={sampleSuggestion} />);
  ...
  it('highlights semantic diffs', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('through the hall, heart pounding.')).toHaveClass('bg-green-200');
  ...
  it('accept updates scene text (history grows)', async () => {
    ...
    expect(await screen.findByTestId('revision-history')).toHaveTextContent('2 revisions');
  });
  ...
  it('renders with accessible roles and keyboard nav', async () => {
    render(<SmartRevisionEngine sceneId="scene1" suggestion={sampleSuggestion} />);
    expect(screen.getByRole('region', { name: /AI revision diff/i })).toBeInTheDocument();
