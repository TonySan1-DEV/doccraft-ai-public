...
const ThemeSummarySidebar: React.FC<any> = ({ report, onSelectTheme }) => (
  <aside className="p-4" aria-label="Theme Summary Sidebar">
    ...
    <ul className="list-disc list-inside text-xs">
      {report.primaryThemes.map((t, i) => <li key={i}><button className="underline text-blue-600" onClick={() => onSelectTheme?.(t)}>{t}</button></li>)}
    </ul>
  </aside>
);
import type { ThemeAlignmentReport, SceneThemeFingerprint, ThemeKeyword } from '../../themeAnalysis/themeTypes';

// Mock data for demo
const mockReport: ThemeAlignmentReport = {
  primaryThemes: ['loyalty', 'sacrifice', 'trust'],
  misalignedScenes: [
    { sceneId: 'scene2', themes: [
      { theme: 'loyalty', strength: 0.2, context: 'He hesitated.' },
      { theme: 'betrayal', strength: 0.7, context: 'She broke her promise.' }
    ] },
    { sceneId: 'scene3', themes: [
  ...
  return (
    <section className="w-full h-full" aria-label="Thematic Analysis Tab">
      <div className="flex flex-col md:grid md:grid-cols-3 gap-4">
        {/* Main content */}
        <main className="md:col-span-2" aria-label="Theme Matrix or Summary" role="region">
          ...
          {loading ? (
            <div className="p-8 text-center text-gray-500" role="status">Loading thematic analysis…</div>
          ) : error ? (
            <div className="p-8 text-center text-red-600" role="alert">{error}</div>
          ) : view === 'matrix' ? (
            <ThemeMatrixPanel
              scenes={selectedTheme ? scenes.filter(s => s.themes.some(t => t.theme === selectedTheme)) : scenes}
            ...
            <div className="p-4" aria-label="Coverage Summary" role="region">
              ...
              <ul className="list-disc list-inside text-xs mb-2">
                {report.primaryThemes.map((t, i) => <li key={i}>{t}</li>)}
              </ul>
              ...
              <div className="mt-4">
                <h3 className="font-semibold text-sm mb-1">Jump to Misaligned Scene:</h3>
