...
// Mock data for now
const mockScenes = [
  { sceneId: 'scene1', themes: [ { theme: 'identity', strength: 0.8, context: 'Who am I?' }, { theme: 'belonging', strength: 0.5, context: 'I want to fit in.' } ] },
  { sceneId: 'scene2', themes: [ { theme: 'betrayal', strength: 0.7, context: 'He lied to me.' } ] },
  { sceneId: 'scene3', themes: [ { theme: 'love', strength: 0.9, context: 'I can’t stop thinking about her.' } ] }
];
const mockThemes = ['identity', 'belonging', 'betrayal', 'love'];

const ThemeViewTab: React.FC = () => {
  const narrativeSync = useNarrativeSync();
  const [activeTab, setActiveTab] = useState<'overview' | 'matrix' | 'coverage'>('matrix');

  return (
    <section className="w-full flex flex-col md:flex-row gap-6 max-w-6xl mx-auto" aria-label="Theme View Tab">
      <div className="flex-1 flex flex-col gap-4">
        <nav className="flex gap-2 mb-2" role="tablist" aria-label="Theme Tabs">
          <button
          ...
          >Theme Coverage</button>
        </nav>
        <div role="tabpanel" aria-labelledby={activeTab} className="mt-2">
          {activeTab === 'matrix' && (
      ...
      <aside className="w-full md:w-80 flex flex-col gap-4" aria-label="Scene Metadata Panel">
        <Suspense fallback={<div className="text-center text-blue-600">Loading scene inspector...</div>}>
          <SceneInspectorPanel narrativeSync={{ currentSceneId: narrativeSync.state.currentSceneId || undefined }} />
        </Suspense>
      </aside>
    </section>
  );
