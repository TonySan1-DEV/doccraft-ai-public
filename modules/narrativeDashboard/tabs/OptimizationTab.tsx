    ...
    {suggestions.map(s => (
      ...
      >
        <div className="flex items-center gap-2 mb-1">
          <span
            className={`px-2 py-0.5 rounded text-xs font-bold ${s.severity === 'High' ? 'bg-red-200 text-red-800' : s.severity === 'Medium' ? 'bg-yellow-200 text-yellow-800' : 'bg-green-200 text-green-800'}`}
          >
            {s.severity}
          </span>
          ...
          <button
            ...
            onClick={() => onSelect(s.targetSceneId)}
          >
            Focus
          </button>
        </div>
...
export const OptimizationTab: React.FC<{
  ...
  const [selectedSceneId, setSelectedSceneId] = useState<string | undefined>(
    narrativeSync.state.currentSceneId
  );
  const [announce, setAnnounce] = useState('');
  const [filter, setFilter] = useState<
    'all' | 'structure' | 'emotion' | 'character'
  >('all');
      ...
      <div className="lg:col-span-3 flex flex-col gap-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2 ml-4">
            <button
              className={`px-2 py-1 rounded text-xs ${filter === 'all' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'}`}
              onClick={() => setFilter('all')}
            >
              All
            </button>
            <button
              className={`px-2 py-1 rounded text-xs ${filter === 'structure' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'}`}
              onClick={() => setFilter('structure')}
            >
              Structure
            </button>
            <button
              className={`px-2 py-1 rounded text-xs ${filter === 'emotion' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'}`}
              onClick={() => setFilter('emotion')}
            >
              Emotion
            </button>
            <button
              className={`px-2 py-1 rounded text-xs ${filter === 'character' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'}`}
        ...
        <Suspense fallback={<div>Loading narrative score...</div>}>
          <NarrativeScoreSummary narrativeSync={narrativeSync.state} />
        </Suspense>
      </div>
      <aside
