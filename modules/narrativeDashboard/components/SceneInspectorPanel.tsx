...
export type OptimizationSuggestion = {
  ...
  impact: number; // 0-100, clamped and validated
  action: string;
};
  ...
  return [
    {
      id: '1',
      message:
        'Structural misalignment: Scene does not match expected plot beat.',
      severity: 'High',
      impact: 90,
...
const SceneInspectorPanel: React.FC<{
  narrativeSync?: { currentSceneId?: string };
}> = ({ narrativeSync }) => {
    ...
    mockFetchSuggestions(sceneId)
      .then(res => {
        if (isMounted) {
          ...
          setAnnounce(
            res.length
              ? `${res.length} suggestions loaded.`
              : 'No suggestions for this scene.'
          );
        }
      })
      .catch(() => {
          ...
          {suggestions.map(s => (
            ...
            >
              <div className="flex items-center gap-2 mb-1">
                <span
                  className={`px-2 py-0.5 rounded text-xs font-bold ${severityColor[s.severity]}`}
                >
                  {s.severity}
                </span>
              ...
              <div className="flex items-center gap-2">
                <div className="flex-1 h-2 bg-gray-200 rounded">
                  <div
                    className={`h-2 rounded ${clamp100(s.impact) > 80 ? 'bg-red-400' : clamp100(s.impact) > 50 ? 'bg-yellow-400' : 'bg-green-400'}`}
                    style={{ width: `${clamp100(s.impact)}%` }}
                    aria-label={`Impact score: ${clamp100(s.impact)}`}
                  />
                </div>
                <span className="text-xs text-gray-600">
                  Impact: {clamp100(s.impact)}
                </span>
              </div>
