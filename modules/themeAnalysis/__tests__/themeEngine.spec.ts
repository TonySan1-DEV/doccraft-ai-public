...
describe('Theme Engine – Thematic Accuracy', () => {
  ...
  it('detects top ThematicSignal[] accurately from sceneText', async () => {
    ...
    const report: ThemeAlignmentReport = await analyzeThemeConsistency(
      scene.text,
      [scene]
    );
    const detectedThemes = report.misalignedScenes.length
      ? report.misalignedScenes[0].themes
      : [];
    const themeNames = detectedThemes.map(t => t.theme);
    expect(themeNames).toEqual(
      expect.arrayContaining(['loyalty', 'betrayal', 'trust', 'hope'])
    );
  ...
  it('flags misalignment and generates suggestions for missing/inverted themes', async () => {
    ...
    // const _comingOfAgeThemes = ['identity', 'belonging', 'rebellion', 'growth'];
    const scene: SceneMeta = {
      sceneId: 's2',
    ...
    const present = report.misalignedScenes.some(
  ...
  it('computes correct coverageScore for scene:theme match', async () => {
    ...
    const ambiguous: SceneMeta = {
      sceneId: 's4',
      text: 'The tone was ironic, nothing was as it seemed.',
    };
    const report2: ThemeAlignmentReport = await analyzeThemeConsistency(
      ambiguous.text,
      [ambiguous]
    );
  ...
  it('handles predefined genre theme sets for drift detection', async () => {
    ...
    const romanceReport = await analyzeThemeConsistency(romanceScene.text, [
      romanceScene,
    ]);
    expect(romanceReport.primaryThemes).toEqual(
      expect.arrayContaining(['love', 'forgiveness', 'jealousy'])
    );
    const noirReport = await analyzeThemeConsistency(noirScene.text, [
      noirScene,
    ]);
    expect(noirReport.primaryThemes).toEqual(
      expect.arrayContaining(['betrayal', 'justice'])
    );
    const thrillerReport = await analyzeThemeConsistency(thrillerScene.text, [
      thrillerScene,
    ]);
    expect(thrillerReport.primaryThemes).toEqual(
      expect.arrayContaining(['paranoia'])
    );
  });
});
