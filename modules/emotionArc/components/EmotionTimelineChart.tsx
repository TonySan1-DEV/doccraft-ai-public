...
const ChartPointComponent = React.memo<{
  point: ChartPoint;
  isSelected: boolean;
  isHovered: boolean;
  ...
  ({
    ...
    const color =
      EMOTION_COLORS[point.emotion as keyof typeof EMOTION_COLORS] || '#6B7280';
...
ChartPointComponent.displayName = 'ChartPointComponent';
...
function EmotionTimelineChart({
  ...
  isLoading = false,
  error = null,
  onCharacterSelect: _onCharacterSelect,
  onBeatClick,
  className = '',
  ...
  //           Math.min(chartData.length - 1, prev + 1)
  //         );
  //         break;
  //       case 'Home':
  //         event.preventDefault();
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === 'development') {
      console.log('[EmotionTimelineChart] context-driven props:', {
        selectedCharacter,
        contextScene,
      });
    }
            ...
            {Array.from(characterBeats.entries()).map(
              ([characterId, beats]) => {
                const avgIntensity =
                  ...
                  beats.length;
                const dominantEmotion = beats.reduce(
                  (acc, beat) => {
