...
export default function TensionCurveViewer({
  ...
  const chartData = useMemo(() => {
    return tensionCurve.map(curve => ({
      ...
      empathy: curve.empathy || 0,
      engagement: curve.engagement || 0,
  ...
  const stats = useMemo(() => {
    ...
    const avgTension =
      chartData.reduce((sum, point) => sum + point.tension, 0) /
      chartData.length;
    const avgEngagement =
      chartData.reduce((sum, point) => sum + point.engagement, 0) /
      chartData.length;
    const avgEmpathy =
      chartData.reduce((sum, point) => sum + point.empathy, 0) /
      chartData.length;
  ...
  const handleKeyDown = useCallback(
    (event: React.KeyboardEvent, point: ChartPoint) => {
    ...
    >
      ...
      >
        ...
        <svg className="absolute inset-0 w-full h-full">
          ...
          <polyline
            points={chartData
              .map(point => `${point.x},${100 - point.tension}`)
              .join(' ')}
            fill="none"
            stroke="#EF4444"
            strokeWidth="2"
        ...
        <svg className="absolute inset-0 w-full h-full">
          <polyline
            points={chartData
              .map(point => `${point.x},${100 - point.engagement}`)
              .join(' ')}
            fill="none"
            stroke="#10B981"
            strokeWidth="2"
        ...
        <svg className="absolute inset-0 w-full h-full">
          <polyline
            points={chartData
              .map(point => `${point.x},${100 - point.empathy}`)
              .join(' ')}
            fill="none"
            stroke="#3B82F6"
            strokeWidth="2"
        ...
        {hoveredPoint && (
          ...
          >
            <div className="font-medium">
              Position: {Math.round(hoveredPoint.x)}%
            </div>
            <div className="text-red-300">
              Tension: {Math.round(hoveredPoint.tension)}%
            </div>
            <div className="text-green-300">
              Engagement: {Math.round(hoveredPoint.engagement)}%
            </div>
            <div className="text-blue-300">
              Empathy: {Math.round(hoveredPoint.empathy)}%
            </div>
          </div>
        )}
      </div>
        ...
        <div className="mt-6 p-4 bg-gray-50 rounded-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm text-gray-700">
                {readerEngagement.highEngagementSections.length} high engagement
                sections
              </span>
            </div>
          ...
          {readerEngagement.predictedDrops.length > 0 && (
            <div className="mt-3 p-3 bg-red-50 border border-red-200 rounded">
              ...
              <div className="text-sm text-red-700 mt-1">
                Consider adding conflict or stakes at{' '}
                {readerEngagement.predictedDrops
                  .map(drop => `${Math.round(drop * 100)}%`)
                  .join(', ')}{' '}
                of the story.
              </div>
            </div>
