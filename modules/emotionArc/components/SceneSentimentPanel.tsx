      ...
      <div className="p-4">
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center space-x-2">
            <span className={`px-2 py-1 text-xs font-medium rounded-full ${getSentimentColor(scene.overallSentiment)}`}>
              {scene.overallSentiment}
            </span>
            <span className={`text-xs font-medium ${getTensionColor(scene.tensionLevel)}`}>
          ...
          <div className="flex flex-wrap gap-2">
            {characterEmotions.map(([characterId, emotion]) => (
              <button
                ...
                aria-label={`View ${characterId}'s emotions: ${emotion.primaryEmotion} (${Math.round(emotion.intensity)}%)`}
              >
                <div 
                  className="w-2 h-2 rounded-full"
                  style={{ backgroundColor: EMOTION_COLORS[emotion.primaryEmotion as keyof typeof EMOTION_COLORS] || '#6B7280' }}
                />
      ...
      <div className="border-t border-gray-100">
        ...
        {isExpanded && (
          ...
          >
            {/* Scene Text Preview */}
            <div>
              <h6 className="text-xs font-medium text-gray-700 mb-2">Scene Preview</h6>
            ...
            <div>
              <h6 className="text-xs font-medium text-gray-700 mb-2">Character Analysis</h6>
              <div className="space-y-3">
                {characterEmotions.map(([characterId, emotion]) => (
                  <div key={characterId} className="bg-white p-3 rounded border">
                    <div className="flex items-center justify-between mb-2">
                      <h7 className="text-sm font-medium text-gray-900">{characterId}</h7>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-xs text-gray-600">Primary Emotion</span>
                        <div className="flex items-center space-x-2">
                          <div 
                      ...
                      {emotion.secondaryEmotions.length > 0 && (
                        <div className="flex items-center justify-between">
                          <span className="text-xs text-gray-600">Secondary Emotions</span>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-sm font-medium">
                          {Math.round(emotion.emotionalComplexity)}%
                        </span>
                      </div>
                      
                      {emotion.contextClues.length > 0 && (
                        <div>
                          <span className="text-xs text-gray-600">Context Clues</span>
                          <div className="mt-1 flex flex-wrap gap-1">
                            {emotion.contextClues.slice(0, 3).map((clue, index) => (
            ...
            {scene.emotionalBeats.length > 0 && (
              <div>
                <h6 className="text-xs font-medium text-gray-700 mb-2">Emotional Beats</h6>
                <div className="space-y-2">
                  {scene.emotionalBeats.map((beat, index) => (
                    <div key={index} className="flex items-center justify-between bg-white p-2 rounded border">
                      <div className="flex items-center space-x-2">
                        <div 
                          className="w-3 h-3 rounded-full"
                          style={{ backgroundColor: EMOTION_COLORS[beat.emotion as keyof typeof EMOTION_COLORS] || '#6B7280' }}
                        />
                        <span className="text-sm font-medium">{beat.characterId}</span>
                        <span className="text-sm text-gray-600 capitalize">{beat.emotion}</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <span className="text-xs text-gray-500">
                          {Math.round(beat.narrativePosition * 100)}% position
                        </span>
            ...
            <div className="bg-gray-100 p-3 rounded">
              <h6 className="text-xs font-medium text-gray-700 mb-2">Analysis Info</h6>
...
export default function SceneSentimentPanel({
  ...
  const [filter, setFilter] = useState<'all' | 'high-tension' | 'low-tension' | 'positive' | 'negative'>('all');
  const [sortBy, setSortBy] = useState<'position' | 'tension' | 'sentiment'>('position');
  ...
  const filteredScenes = useMemo(() => {
    ...
    switch (filter) {
      ...
      case 'positive':
        scenes = scenes.filter(scene => 
          ['positive', 'joy', 'love'].includes(scene.overallSentiment.toLowerCase())
        );
        break;
      case 'negative':
        scenes = scenes.filter(scene => 
          ['negative', 'fear', 'anger', 'sadness', 'disgust'].includes(scene.overallSentiment.toLowerCase())
        );
        break;
    }
    ...
    switch (sortBy) {
      ...
      case 'tension':
        ...
        break;
      case 'sentiment':
        scenes.sort((a, b) => a.overallSentiment.localeCompare(b.overallSentiment));
        break;
    }
    ...
    return (
      ...
      >
        <div className="flex items-center">
          ...
          <span className="text-red-800 font-medium">Error loading scene data</span>
        </div>
    ...
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-blue-50 p-3 rounded-lg">
          ...
          <div className="text-xl font-bold text-blue-900">
            {Math.round(sceneData.reduce((sum, scene) => sum + scene.tensionLevel, 0) / sceneData.length)}%
          </div>
        </div>
        
        <div className="bg-green-50 p-3 rounded-lg">
          <div className="text-sm font-medium text-green-600">Positive Scenes</div>
        ...
        <div className="bg-red-50 p-3 rounded-lg">
          <div className="text-sm font-medium text-red-600">Negative Scenes</div>
        ...
        <div className="bg-purple-50 p-3 rounded-lg">
          <div className="text-sm font-medium text-purple-600">Total Characters</div>
          <div className="text-xl font-bold text-purple-900">
            {new Set(sceneData.flatMap(scene => Array.from(scene.characterEmotions.keys()))).size}
          </div>
        </div>
      </div>
      ...
      {filteredScenes.length === 0 && (
        <div className="text-center py-8">
          ...
          <p className="mt-2 text-gray-500">No scenes match the current filter</p>
        </div>
      )}
    </div>
  );
