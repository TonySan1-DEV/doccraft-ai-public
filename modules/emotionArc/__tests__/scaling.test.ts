...
describe('Scaling Invariants', () => {
  ...
  describe('Display Utilities', () => {
    it('should format position as percentage display', () => {
      expect(toPercentDisplay(0.25)).toBe('25%');
      expect(toPercentDisplay(0.5)).toBe('50%');
      expect(toPercentDisplay(0.75)).toBe('75%');
      expect(toPercentDisplay(0)).toBe('0%');
      expect(toPercentDisplay(1)).toBe('100%');
    });

    it('should assert 0-100 range and warn on violations', () => {
      const consoleSpy = jest.spyOn(console, 'warn').mockImplementation();

      // Valid values should not warn
      expect(assert0to100(50)).toBe(50);
      expect(assert0to100(0)).toBe(0);
      expect(assert0to100(100)).toBe(100);
      expect(consoleSpy).not.toHaveBeenCalled();

      // Invalid values should warn
      expect(assert0to100(-10)).toBe(-10);
      expect(assert0to100(150)).toBe(150);
      expect(consoleSpy).toHaveBeenCalledTimes(2);

      consoleSpy.mockRestore();
    });

    it('should use custom labels in assertions', () => {
      const consoleSpy = jest.spyOn(console, 'warn').mockImplementation();

      assert0to100(-5, 'intensity');
      expect(consoleSpy).toHaveBeenCalledWith(
        '[scale] intensity out of range 0–100:',
        -5
      );
