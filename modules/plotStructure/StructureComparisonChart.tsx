...
import type { ArcPlotOverlay, EmotionalGapAnalysis, EmotionalTensionAnalysis } from './structureSuggestionEngine';
    ...
    >
            ...
            <div key={beat.id} className="flex items-center">
              ...
              {overlay && (
                <div className={`ml-2 px-2 py-1 rounded text-xs ${
                  overlay.alignment === 'strong' ? 'bg-blue-100 text-blue-700' :
                  overlay.alignment === 'moderate' ? 'bg-yellow-100 text-yellow-700' :
                  overlay.alignment === 'weak' ? 'bg-orange-100 text-orange-700' :
      ...
      {emotionalOverlays.length > 0 && (
        <div className="mt-4 p-3 bg-gray-50 rounded">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div>
              <span className="font-medium">Strong:</span> {emotionalOverlays.filter(o => o.alignment === 'strong').length}
            </div>
            <div>
              <span className="font-medium">Mismatch:</span> {emotionalOverlays.filter(o => o.alignment === 'mismatch').length}
            </div>
            <div>
              <span className="font-medium">Avg Tension:</span> {Math.round(emotionalOverlays.reduce((sum, o) => sum + o.tensionLevel, 0) / emotionalOverlays.length)}
            </div>
            <div>
              <span className="font-medium">Avg Engagement:</span> {Math.round(emotionalOverlays.reduce((sum, o) => sum + o.engagementLevel, 0) / emotionalOverlays.length)}
            </div>
          </div>
        </div>
      )}
      ...
      {emotionalGaps.length > 0 && (
        <div className="mt-4 p-3 bg-red-50 rounded">
          <h4 className="text-sm font-medium mb-2 text-red-700">Emotional Gaps</h4>
          <div className="space-y-1">
            {emotionalGaps.map((gap, index) => (
              <div key={index} className="text-xs text-red-600">
                <span className="font-medium">{gap.gapType}:</span> {gap.description}
              </div>
            ))}
          </div>
        </div>
      ...
      {tensionAnalysis.length > 0 && (
        <div className="mt-4 p-3 bg-blue-50 rounded">
          <h4 className="text-sm font-medium mb-2 text-blue-700">Tension Consistency</h4>
          <div className="space-y-1">
            {tensionAnalysis.map((analysis, index) => (
              <div key={index} className="text-xs">
                <span className="font-medium">{analysis.act}:</span> {analysis.consistency} 
                ({Math.round(analysis.actualTension)} vs {analysis.expectedTension})
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="mt-2 text-xs text-gray-500">Green: Present in story. Red: Missing in story.</div>
    </div>
  );
} 