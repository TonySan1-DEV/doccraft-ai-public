...
import type { PlotStructureAnalysis, PlotGap, ScenePlacementSuggestion, PlotBeat } from './initPlotEngine';
import type { EmotionalBeat, ArcSimulationResult, TensionCurve, SceneEmotionData } from '../emotionArc/types/emotionTypes';
...
export interface EmotionalGapAnalysis {
  position: number;
  gapType: 'tension_drop' | 'empathy_drop' | 'engagement_drop' | 'complexity_spike';
...
export class StructureSuggestionEngine {
    ...
    sceneData.forEach((scene, index) => {
      ...
      if (beat) {
        ...
        const overlay: ArcPlotOverlay = {
          ...
          emotionalBeat,
          tensionLevel: tensionCurve.tension,
          empathyLevel: tensionCurve.empathy,
          engagementLevel: tensionCurve.engagement,
          emotionalComplexity: tensionCurve.emotionalComplexity,
          alignment: this.calculateAlignment(tensionCurve, beat),
          suggestions: this.generateEmotionalSuggestions(tensionCurve, beat)
        };
  ...
  analyzeTensionConsistency(
    ...
    Object.entries(actGroups).forEach(([act, actOverlays]) => {
      ...
      const avgTension = actOverlays.reduce((sum, o) => sum + o.tensionLevel, 0) / actOverlays.length;
      ...
      const consistency = this.determineConsistency(avgTension, expectation, variance);
      const recommendations = this.generateTensionRecommendations(avgTension, expectation, consistency);
  ...
  generateEmotionalImprovements(
    ...
    const weakAlignments = overlays.filter(o => o.alignment === 'weak' || o.alignment === 'mismatch');
    if (weakAlignments.length > 0) {
      suggestions.push(`Found ${weakAlignments.length} scenes with weak emotional alignment to plot structure.`);
    }
    
    // Address gaps
    gaps.forEach(gap => {
      if (gap.severity === 'high') {
        suggestions.push(`Critical gap at ${Math.round(gap.position * 100)}%: ${gap.description}`);
      }
    });
    ...
    tensionAnalysis.forEach(analysis => {
      if (analysis.consistency === 'problematic') {
        suggestions.push(`${analysis.act} tension (${Math.round(analysis.actualTension)}) significantly differs from expected (${analysis.expectedTension}).`);
      }
    });
  ...
  generateStructuralAIWarnings(
    ...
    if (protagonist && frameworkBeats) {
      ...
      if (climaxOverlay && protagonist.flaw && protagonist.flawResolved === false) {
        warnings.push({
          ...
          severity: 'critical',
          impact: 9,
          suggestion: 'Clarify flaw resolution in climax or adjust character arc.'
        });
      }
    }

    // 2. Final act has flat empathy score
    const finalActOverlays = overlays.filter(o => ['road', 'climax', 'resurrection', 'return'].includes(o.beatId));
    if (finalActOverlays.length > 0) {
      const avgEmpathy = finalActOverlays.reduce((sum, o) => sum + o.empathyLevel, 0) / finalActOverlays.length;
      if (avgEmpathy < 30) {
        finalActOverlays.forEach(o => {
          warnings.push({
            ...
            severity: 'warning',
            impact: 7,
            suggestion: 'Add character vulnerability or deepen relationships in final act.'
          });
        });
      }
    }
    ...
    if (funAndGamesOverlay && funAndGamesOverlay.tensionLevel < 30) {
      warnings.push({
        ...
        severity: 'warning',
        impact: 6,
        suggestion: 'Introduce conflict, surprise, or challenge in this section.'
      });
    }
    ...
    overlays.forEach(o => {
      if (o.alignment === 'mismatch') {
        warnings.push({
          ...
          severity: 'warning',
          impact: 8,
          suggestion: 'Reassign scene to a more fitting beat or revise emotional content.'
        });
      }
    });
  ...
  private findBeatAtPosition(beats: PlotBeat[], position: number): PlotBeat | null {
  ...
  private createEmotionalBeat(scene: SceneEmotionData, position: number): EmotionalBeat {
    // Create emotional beat from scene data
    const avgIntensity = Array.from(scene.characterEmotions.values())
      .reduce((sum, emotion: any) => sum + emotion.intensity, 0) / scene.characterEmotions.size;
    
    return {
      sceneId: scene.sceneId,
      characterId: Array.from(scene.characterEmotions.keys())[0] || '',
      emotion: scene.overallSentiment,
      intensity: avgIntensity,
      narrativePosition: position,
      timestamp: Date.now(),
      context: scene.sceneText.substring(0, 100)
    };
  }

  private calculateAlignment(tensionCurve: TensionCurve, beat: PlotBeat): 'strong' | 'moderate' | 'weak' | 'mismatch' {
    // Determine alignment based on beat type and emotional intensity
    const isHighTensionBeat = ['climax', 'crisis', 'confrontation'].includes(beat.id);
    const isLowTensionBeat = ['resolution', 'denouement', 'setup'].includes(beat.id);
  ...
  private generateEmotionalSuggestions(tensionCurve: TensionCurve, beat: PlotBeat): string[] {
    const suggestions: string[] = [];
    ...
    if (tensionCurve.engagement < 50) {
      suggestions.push('Risk of reader disengagement - consider adding stakes or conflict');
    }
  ...
  private isInAct2(beatId: string): boolean {
    return ['refusal', 'meeting', 'tests', 'approach', 'ordeal', 'reward'].includes(beatId);
  }
  ...
  private analyzeActGaps(overlays: ArcPlotOverlay[], actName: string): EmotionalGapAnalysis[] {
    const gaps: EmotionalGapAnalysis[] = [];
    
    if (overlays.length === 0) return gaps;
    
    // Check for tension drops
    const avgTension = overlays.reduce((sum, o) => sum + o.tensionLevel, 0) / overlays.length;
    if (avgTension < 30) {
      gaps.push({
        position: overlays[0].emotionalBeat.narrativePosition,
        gapType: 'tension_drop',
        severity: 'high',
        description: `${actName} has low average tension (${Math.round(avgTension)})`,
        suggestedImprovements: ['Add conflict', 'Increase stakes', 'Introduce obstacles']
      });
    }
    
    // Check for engagement drops
    const avgEngagement = overlays.reduce((sum, o) => sum + o.engagementLevel, 0) / overlays.length;
    if (avgEngagement < 40) {
      gaps.push({
        position: overlays[0].emotionalBeat.narrativePosition,
        ...
        suggestedImprovements: ['Add character development', 'Increase pacing', 'Introduce new elements']
      });
    }
  ...
  private generateTensionRecommendations(
    ...
    if (consistency === 'problematic') {
      if (actualTension < expectation.min) {
        recommendations.push('Significantly increase tension to meet act expectations');
      } else if (actualTension > expectation.max) {
        recommendations.push('Consider reducing tension to avoid overwhelming readers');
      }
    } else if (consistency === 'inconsistent') {
      recommendations.push('Fine-tune tension to better align with act structure');
    }
