...
import type { SceneEmotionData, EmotionalBeat } from '../../emotionArc/types/emotionTypes';
...
describe('StructureSuggestionEngine emotional integration', () => {
  ...
  it('maps emotional beats to framework beats', () => {
    const overlays = engine.mapEmotionalBeatsToFramework(mockSceneData, frameworkBeats);
  ...
  it('generates improvement suggestions for emotional misalignments', () => {
    ...
    const suggestions = engine.generateEmotionalImprovements(overlays, gaps, tensionAnalysis);
  ...
  it('calculates proper alignment between emotional intensity and beat type', () => {
    ...
    overlays.forEach(overlay => {
      expect(['strong', 'moderate', 'weak', 'mismatch']).toContain(overlay.alignment);
