...
const mockPersona: CharacterPersona = {
  ...
  personality: 'Openness: high, Conscientiousness: high, Extraversion: moderate, Agreeableness: high, Neuroticism: low',
  knownConnections: [
    { name: 'Dr. Kalin', relationship: 'rival', description: 'Long-standing academic adversary' },
  ],
  background: 'A retired knight and scholar.',
  traits: { tone: 'formal', quirks: 'quotes ancient proverbs', worldview: 'optimistic' },
  memory: {},
...
const mockSimulateCharacterChat = simulateChatModule.simulateCharacterChat as jest.Mock;
...
describe('CharacterChatPanel', () => {
  ...
  it('renders persona summary and empty chat for Pro user', () => {
    render(
      <AuthContext.Provider value={{ user: proUser }}>
        <CharacterChatPanel persona={mockPersona} contextText="A dark and stormy night." />
      </AuthContext.Provider>
    );
    expect(screen.getByText('Sir Reginald')).toBeInTheDocument();
    expect(screen.getByText('Mentor')).toBeInTheDocument();
    expect(screen.getByText('A retired knight and scholar.')).toBeInTheDocument();
    expect(screen.getByPlaceholderText(/Speak to Sir Reginald/i)).toBeInTheDocument();
    expect(screen.getByText(/Start a conversation/i)).toBeInTheDocument();
  });

  it('disables input and shows upgrade CTA for Free user', () => {
    render(
    ...
    expect(screen.getByText(/Upgrade to Pro to unlock character chat/i)).toBeInTheDocument();
  });

  it('sends user message and displays AI reply (Pro user)', async () => {
    ...
    expect(await screen.findByText('Verily, thou hast spoken with courage!')).toBeInTheDocument();
    // Check sender labels
    expect(screen.getAllByText('You').length).toBeGreaterThan(0);
    expect(screen.getAllByText('Sir Reginald').length).toBeGreaterThan(1);
  });

  it('shows typing indicator while waiting for AI', async () => {
    ...
    resolve && resolve('Indeed, I am ever present.');
    await waitFor(() =>
      expect(screen.getByText('Indeed, I am ever present.')).toBeInTheDocument()
    );
  });

  it('handles AI fallback and error gracefully', async () => {
    ...
    fireEvent.click(screen.getByText('Send'));
    expect(await screen.findByText(/Failed to get character reply/i)).toBeInTheDocument();
  });

  it('remembers previous messages (memory enforcement)', async () => {
    ...
    fireEvent.click(screen.getByText('Send'));
    expect(await screen.findByText('A second wise answer.')).toBeInTheDocument();
