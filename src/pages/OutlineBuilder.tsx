    ...
    <div className="max-w-2xl mx-auto py-10 px-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 bg-white dark:bg-zinc-900 rounded-lg shadow p-6">
        ...
        <button
          ...
          disabled={loading}
        >
          {loading && <Loader2 className="animate-spin w-5 h-5" />} Generate Outline
        </button>
      </form>
      <div className="mt-8">
        {error && <div className="text-red-600 mb-4">{error}</div>}
        {!loading && outline.length === 0 && !error && (
          <div className="text-zinc-500 text-center">Enter your book details to generate an outline.</div>
        )}
        {outline.map((chapter, idx) => (
          <div key={idx} className="mb-4 border rounded-lg bg-white dark:bg-zinc-900 shadow">
            <button
