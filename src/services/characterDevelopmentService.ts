  ...
  return {
    ...
    relationshipPatterns: [
      `Based on their ${character.archetype} nature, they likely form ${
        (character.knownConnections?.length ?? 0) > 0 ? 'deep' : 'selective'
      } relationships.`,
...
export function generatePersonalityInsights(
  ...
  // MBTI analysis
  const mbtiTrait = character.traits?.find(
    t => t.name === 'mbti' || t.category === 'psychological'
  );
  if (mbtiTrait) {
    const mbtiType = mbtiTrait.value;
  ...
  // Enneagram analysis
  const enneagramTrait = character.traits?.find(t => t.name === 'enneagram');
  if (enneagramTrait) {
    const enneagramType = enneagramTrait.value;
  ...
  // Big Five analysis
  const bigFiveTrait = character.traits?.find(t => t.name === 'bigFive');
  if (bigFiveTrait) {
    const bigFive = JSON.parse(bigFiveTrait.value);
