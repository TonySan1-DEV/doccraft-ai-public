...
} from '../services/characterRelationshipEngine';
...
export default function AdvancedCharacterDevelopment({
  ...
  const loadCharacterData = useCallback(async () => {
    ...
    try {
      ...
      const characterRelationships =
        await characterRelationshipEngine.getAllRelationships(character.id);
      setRelationships(characterRelationships);

      // Generate insights
      const personalityInsights = await advancedCharacterAI.deepenPersonality(
        character.id
      );
      const conflictInsights =
        await advancedCharacterAI.generateInternalConflict(character.id);
      const growthInsights = await advancedCharacterAI.suggestCharacterGrowth(
        character.id
      );
  ...
  const generateDeepInsight = useCallback(
    async (category: string) => {
      ...
      try {
        ...
        switch (category) {
          case 'personality':
            newInsights = await advancedCharacterAI.deepenPersonality(
              character.id
            );
            break;
          case 'conflicts':
            newInsights = await advancedCharacterAI.generateInternalConflict(
              character.id
            );
            break;
          case 'growth':
            newInsights = await advancedCharacterAI.suggestCharacterGrowth(
              character.id
            );
            break;
          case 'relationships': {
            const relationshipPrompts = await Promise.all(
              relationships.map(rel =>
                characterRelationshipEngine.generateRelationshipPrompts(rel.id)
              )
            );
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-blue-600">
                {Math.round(characterMetrics.personalityDepth * 100)}%
              </p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-purple-600">
                {Math.round(characterMetrics.growthPotential * 100)}%
              </p>
            </div>
        ...
        >
          ...
          <div className="text-left">
            ...
            <p className="text-sm opacity-90">
              Explore character&apos;s inner world
            </p>
          </div>
        </button>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border">
        ...
        <div className="space-y-3">
          {insights
            .filter(
              insight =>
                insight.includes('personality') ||
                insight.includes('belief') ||
                insight.includes('value')
            )
            .map((insight, index) => (
              <div
                key={index}
            ...
            <div key={index} className="p-4 border rounded-lg">
              <div className="flex items-center justify-between mb-2">
                <h4 className="font-medium">
                  {relationship.characterA === character.id
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  <p className="text-gray-600">
                    Conflict: {Math.round(relationship.conflict * 100)}%
                  </p>
                  <p className="text-gray-600">
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              Character is {Math.round(characterMetrics.evolutionStage * 100)}%
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="space-y-2">
                {[
                  'Overcame initial fears',
                  'Built first meaningful relationship',
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border">
        ...
        <div className="space-y-3">
          {insights
            .filter(
              insight =>
                insight.includes('grow') ||
                insight.includes('develop') ||
                insight.includes('improve')
            )
            .map((insight, index) => (
              <div
                key={index}
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div
                  ...
                  style={{
                    width: `${characterMetrics.personalityDepth * 100}%`,
                  }}
                ></div>
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between text-sm mb-1">
                ...
                <span>
                  {Math.round(characterMetrics.emotionalRange * 100)}%
                </span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div
                  className="bg-green-500 h-2 rounded-full"
                  style={{ width: `${characterMetrics.emotionalRange * 100}%` }}
                ></div>
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between text-sm mb-1">
                ...
                <span>
                  {Math.round(characterMetrics.consistencyScore * 100)}%
                </span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div
                  ...
                  style={{
                    width: `${characterMetrics.consistencyScore * 100}%`,
                  }}
                ></div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-blue-700">
                  Focus: {developmentSession.focus}
                </p>
