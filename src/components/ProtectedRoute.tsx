  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      if (
        ...
        toast.error(
          `This feature requires a higher tier. You are currently on ${user.tier} tier.`
        );
      ...
      const redirectInfo = getUserRedirectPath(user);
      if (
        redirectInfo.path !== location.pathname &&
        location.pathname === '/'
      ) {
        navigate(redirectInfo.path);
        return;
    ...
    try {
      ...
      if (data) {
        ...
        if (data.account_status === 'paused') {
          const pauseEndDate = data.pause_end_date
          ...
          if (!isExpired) {
            toast.error(
              'Your account is currently paused. Please wait until the pause period ends.'
            );
            navigate('/');
            return;
          }
